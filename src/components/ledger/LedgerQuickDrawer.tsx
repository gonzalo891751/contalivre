            ...
            {account && (
                <>
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        onClick={onClose}
                        className="fixed inset-0 bg-slate-200/60 backdrop-blur-sm z-40"
                        aria-hidden="true"
                    />
                    ...
                    <motion.aside
                        ...
                        className="fixed top-0 right-0 h-full w-full md:w-[480px] bg-white shadow-2xl z-50 flex flex-col border-l border-slate-200"
                        role="dialog"
                        aria-labelledby="drawer-title"
                        aria-modal="true"
                    >
                        <div className="p-6 border-b border-slate-100 bg-white/80 backdrop-blur sticky top-0">
                            <div className="flex justify-between items-start mb-2">
                                <span className="font-mono text-xs text-blue-500 font-medium bg-blue-50 px-2 py-1 rounded">
                                    {account.code}
                                </span>
                                <button
                                    onClick={onClose}
                            ...
                            <h2 id="drawer-title" className="text-2xl font-bold font-display text-slate-900 mb-1">
                                {account.name}
                            </h2>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-8">
                            <div className="p-6 rounded-2xl bg-slate-50 border border-slate-100 text-center relative overflow-hidden group">
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm font-semibold text-slate-900">Resumen del Periodo</h3>
                                    ...
                                    {account.lastMovements.length > 0 ? (
                                        account.lastMovements.map((mov, i) => (
                                            <div key={`${mov.entryId}-${i}`} className="relative">
                                                <div className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full bg-white border-2 border-slate-300" />

                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <p className="text-xs text-slate-400 mb-0.5">{mov.date}</p>
                                                        <p className="text-sm font-medium text-slate-800">{mov.memo}</p>
                                                    </div>
                        ...
                        <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex flex-col gap-3">
                            ...
                            <button
                                className="w-full py-3 bg-white border border-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"
