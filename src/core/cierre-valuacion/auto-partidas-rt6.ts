...
/**
 * Filter out manual RT6 inventory adjustments from ledger movements.
 * These are movements generated by the Inventario module's manual RT6 feature
 * and should not be re-processed by auto-partidas to avoid double counting.
 */
function filterManualRT6InventoryMovements(movements: LedgerMovement[]): LedgerMovement[] {
    const manualRT6Pattern = /ajuste por inflacion \(manual\)/i;
    return movements.filter(m => {
        const normalizedMemo = normalizeText(m.memo || '');
        return !manualRT6Pattern.test(normalizedMemo);
    });
}
...
function generatePartidaForAccount(
    ...
    if (manualOriginDate) {
        ...
        // Filter out manual RT6 inventory adjustments to avoid double counting
        const filteredMovements = filterManualRT6InventoryMovements(balance.movements);
