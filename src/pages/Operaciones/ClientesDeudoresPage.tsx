/**
 * ClientesDeudoresPage - Gestión de Clientes / Deudores por Ventas
 *
 * Pantalla espejo de ProveedoresAcreedoresPage.
 * Toggle: Deudores por Ventas (1.1.02.01) / Documentos a Cobrar (1.1.02.02)
...
const DOCUMENTOS_COBRAR_CODE = '1.1.02.02'
...
interface TerceroRow {
    ...
    name: string
    code: string
    balance: number // positive = client owes us (debit normal side for asset)
...
export default function ClientesDeudoresPage() {
    ...
    const controlCode = DEUDORES_CODE
    ...
    const terceroRows = useMemo((): TerceroRow[] => {
        ...
        }).filter(t => t.balance !== 0 || true)
    }, [balances, childAccounts, movements])

    // Build pending docs (SALE movements with saldo > 0)
    const pendingDocs = useMemo((): PendingDoc[] => {
        ...
        // Also check parent control accounts (deudores + documentos) for all-clients view
        const allDeudoresChildren = accounts.filter(a => {
            const deudoresControl = accounts.find(c => c.code === DEUDORES_CODE)
            const docsCobrarControl = accounts.find(c => c.code === DOCUMENTOS_COBRAR_CODE)
            return (
                (deudoresControl && a.parentId === deudoresControl.id && !a.isHeader) ||
                (docsCobrarControl && a.parentId === docsCobrarControl.id && !a.isHeader)
            )
        })
        const allClientNames = new Set(allDeudoresChildren.map(a => normalizeForCompare(a.name)))
        ...
        const sales = movements.filter(m =>
            ...
            return childAccountNames.has(norm) || allClientNames.has(norm)
        })

        // Calculate cobros for each sale by sourceMovementId
        const cobrosBySource = new Map<string, number>()
        movements
            .filter(m => m.type === 'PAYMENT' && m.paymentDirection === 'COBRO' && m.sourceMovementId)
            .forEach(m => {
                const current = cobrosBySource.get(m.sourceMovementId!) || 0
                cobrosBySource.set(m.sourceMovementId!, current + m.total)
            })

        // Legacy cobros by notes/reference
        movements
            .filter(m => m.type === 'PAYMENT' && m.paymentDirection === 'COBRO' && !m.sourceMovementId && m.notes)
            .forEach(m => {
                const ref = m.notes || ''
                const current = cobrosBySource.get(ref) || 0
                cobrosBySource.set(ref, current + m.total)
            })
        ...
        for (const sale of sales) {
            const paidByLink = cobrosBySource.get(sale.id) || 0
            const paidByRef = sale.reference ? (cobrosBySource.get(sale.reference) || 0) : 0
            const reclassed = reclassBySource.get(sale.id) || 0
            const totalPaid = paidByLink + paidByRef
            const saldoPendiente = sale.total - totalPaid - reclassed

            if (saldoPendiente <= 0.01) continue

            const norm = normalizeForCompare(sale.counterparty!)
            docs.push({
                movementId: sale.id,
                date: sale.date,
    ...
    const reclassInstruments = useMemo((): ReclassInstrument[] => {
        if (!movements) return []
        return movements
            .filter(m => m.type === 'RECLASS' && m.paymentMethod === 'clientes' && m.counterparty)
            .map(m => ({
                movementId: m.id,
    ...
    // KPIs
    const totalReceivable = useMemo(() => terceroRows.reduce((sum, t) => sum + t.balance, 0), [terceroRows])
    ...
    // Navigation handlers
    const handleCobrar = useCallback((terceroName?: string, sourceMovementId?: string) => {
        navigate('/operaciones/inventario', {
            state: {
                prefillTab: 'pagos',
                prefillPaymentDirection: 'COBRO',
    ...
    const handleSavePerfeccionar = useCallback(async (data: PerfeccionarSaveData) => {
        ...
        // Build paymentSplits from the split rows
        ...
        // Use first split's instrument info for backward compat on the movement level
        ...
        await createBienesMovement({
            ...
            paymentMethod: 'clientes',
            paymentSplits,
            autoJournal: true,
            linkedJournalEntryIds: [],
            notes: data.notes || `Perfeccionamiento credito - ${perfeccionarTercero}`,
    ...
    // --- Config ---
    const moduleTitle = 'Clientes (Deudores por Ventas)'
    const moduleDesc = 'Gestiona tus ventas a crédito, cuentas corrientes y cobros.'
    const ctaLabel = 'Registrar Venta'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 fade-in">
            ...
            {activeTab === 'listado' && (
                <ListadoTab
                    ...
                    onNew={handleNewSale}
                    ctaLabel="Nuevo Cliente (desde Ventas)"
                    pendingDocs={pendingDocs}
        ...
        <div className="fade-in space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="text-slate-500 text-sm font-medium">Documentos a Cobrar</div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="text-slate-500 text-sm font-medium">
                        Saldo a Cobrar <span className="text-[10px] text-slate-400 font-normal">(Mayor)</span>
                    </div>
                    <div className="font-mono text-2xl font-bold text-slate-900 mt-1 tabular-nums">{fmtCurrency(totalReceivable)}</div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="bg-orange-50 text-orange-600 px-2 py-1 rounded text-xs font-semibold border border-orange-200">Critico</span>
                    </div>
                    <div className="text-slate-500 text-sm font-medium">Vencido (+90 dias)</div>
                    <div className="font-mono text-2xl font-bold text-slate-900 mt-1 tabular-nums">{fmtCurrency(overdueOver90)}</div>
                </div>
            </div>

            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="font-display font-semibold text-lg mb-6">Antiguedad de Creditos (Aging)</h3>
                <div className="flex flex-col gap-4">
                    {[
                        { label: '0-30d', value: aging.d30, color: 'bg-emerald-500' },
                        { label: '31-60d', value: aging.d60, color: 'bg-emerald-300' },
...
function ListadoTab({
    ...
    onCobrar: (name: string, sourceMovementId?: string) => void
    ...
    // Group pendingDocs by tercero (normalized name)
    ...
    // Group RECLASS instruments by source comprobante
    ...
    // Unlinked instruments (no sourceMovementId) grouped by tercero
    ...
    return (
        <div className="fade-in space-y-4">
            <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 flex items-start gap-3">
                <Info size={18} weight="fill" className="text-emerald-600 mt-0.5 shrink-0" />
                <p className="text-xs text-emerald-900">
                    <strong>Importante:</strong> Los clientes se crean automaticamente desde Ventas/Cobros.
                    Los saldos se calculan desde el Libro Mayor (subcuenta control).
                    Expandi cada cliente para ver comprobantes e instrumentos.
                </p>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <div className="relative w-full md:w-80">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-emerald-500 shadow-sm"
                    />
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200 text-xs text-slate-500 uppercase tracking-wider font-semibold">
                                ...
                                <th className="px-4 py-4">Cliente / Razon Social</th>
                                ...
                                <tr>
                                    <td colSpan={6} className="py-12 text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-slate-500 text-xs">Registra una venta para crear un cliente automaticamente.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                terceros.map(t => {
                                    ...
                                    return (
                                        <React.Fragment key={t.accountId}>
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-4">
                                                    <div className="flex items-center">
                                                        <div className="h-8 w-8 rounded bg-emerald-50 flex items-center justify-center text-emerald-600 font-bold mr-3 text-xs">
                                                        ...
                                                        {t.balance > 0 && (
                                                            <>
                                                                <button
                                                                    ...
                                                                    className="bg-white border border-slate-200 text-emerald-600 text-xs font-semibold px-3 py-1.5 rounded hover:bg-emerald-50 hover:border-emerald-200 transition-colors flex items-center gap-1"
                                                                    title="Perfeccionar credito (pasar a documentado)"
                                                                >
                                                                    <ArrowsLeftRight size={12} weight="bold" /> Perfeccionar
                                                                </button>
                                                                <button
                                                                    onClick={() => onCobrar(t.name)}
                                            ...
                                            {isExpanded && terceroDocs.map(doc => {
                                                ...
                                                return (
                                                    <React.Fragment key={`doc-${doc.movementId}`}>
                                                        {/* Comprobante row */}
                                                        <tr className="bg-emerald-50/30 border-t border-slate-100">
                                                            ...
                                                            <td className="px-4 py-2 text-right">
                                                                <div className="flex items-center justify-end gap-1.5">
                                                                    <button
                                                                        ...
                                                                        className="text-emerald-600 text-[10px] font-semibold px-2 py-1 rounded hover:bg-emerald-50 transition-colors flex items-center gap-1"
                                                                        title="Perfeccionar este comprobante"
                                                                    >
                                                                        <ArrowsLeftRight size={10} weight="bold" /> Perfeccionar
                                                                    </button>
                                                                    <button
                                                                        onClick={() => onCobrar(t.name, doc.movementId)}
                                                        ...
                                                        {/* Instruments born from this comprobante */}
                                                        {linkedInstruments.map(inst => {
                                                            const InstIcon = inst.instrumentType === 'PAGARE' ? Scroll
                                                                ...
                                                                : Receipt
                                                            return (
                                                                <tr key={`inst-${inst.movementId}`} className="bg-emerald-50/15 border-t border-dashed border-slate-100">
                                                                    <td className="px-3 py-1.5" />
                                                                    <td className="px-4 py-1.5" colSpan={2}>
                                                                        <div className="flex items-center gap-2 pl-12">
                                                                            <InstIcon size={12} className="text-emerald-500 shrink-0" />
                                                                    ...
                                                                    <td className="px-4 py-1.5 text-right">
                                                                        <span className="font-mono text-[10px] tabular-nums text-emerald-600">{fmtCurrency(inst.amount)}</span>
                                                                    </td>
                                            ...
                                            {/* Unlinked instruments (RECLASS without sourceMovementId) */}
                                            {isExpanded && terceroUnlinked.map(inst => {
                                                const InstIcon = inst.instrumentType === 'PAGARE' ? Scroll
                                                    : (inst.instrumentType === 'ECHEQ' || inst.instrumentType === 'CHEQUE') ? Bank
                                                    : Receipt
                                                return (
                                                    <tr key={`uinst-${inst.movementId}`} className="bg-emerald-50/15 border-t border-dashed border-slate-100">
                                                        <td className="px-3 py-1.5" />
                                                        <td className="px-4 py-1.5" colSpan={2}>
                                                            <div className="flex items-center gap-2 pl-6">
                                                                <InstIcon size={12} className="text-emerald-500 shrink-0" />
        ...
        return (
            <div className="fade-in">
                <div className="bg-white p-12 text-center rounded-xl border border-slate-200 shadow-sm">
                    <ListDashes size={48} className="text-emerald-600 mx-auto mb-4" weight="duotone" />
                    <h3 className="font-display font-semibold text-lg text-slate-900">Historial de Movimientos</h3>
                    <p className="text-slate-500 max-w-md mx-auto mt-2">
                        Aca vas a ver todas las facturas, cobros y ajustes realizados historicamente.
                    </p>
                </div>
            </div>
        )
    ...
    return (
        <div className="fade-in space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="font-display font-semibold text-lg">Agenda de Cobros</h2>
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="text-right">
                                ...
                                <button
                                    onClick={() => onCobrar(doc.counterparty, doc.movementId)}
...
function StatusBadge({ status }: { status: TerceroRow['status'] }) {
    const config = {
        ok: { bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-200', label: 'Al Dia' },
