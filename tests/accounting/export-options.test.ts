...
describe('Fase 2D — opciones de exportación', () => {
    ...
    it('filtra las hojas por el contenido seleccionado', () => {
        const opts: ExportEstadosOptions = { ...defaultExportOptions(false), content: { esp: true, er: false, eepn: false, efe: false, notas: false, indicadores: false, analisis: false } }
    ...
    it('elige la hoja del EFE según el método', () => {
        const direct = buildSelectedReportSheets(bundle, { ...defaultExportOptions(false), content: { esp: false, er: false, eepn: false, efe: true, notas: false, indicadores: false, analisis: false }, efeMethod: 'DIRECT' })
        expect(direct.some(s => s.name === 'EFE directo')).toBe(true)
        const indirect = buildSelectedReportSheets(bundle, { ...defaultExportOptions(false), content: { esp: false, er: false, eepn: false, efe: true, notas: false, indicadores: false, analisis: false }, efeMethod: 'INDIRECT' })
        expect(indirect.some(s => s.name === 'EFE indirecto')).toBe(true)
    })

    it('la hoja ESP exportada coincide con las cifras del bundle', () => {
        const opts = { ...defaultExportOptions(false), content: { esp: true, er: false, eepn: false, efe: false, notas: false, indicadores: false, analisis: false } }
    ...
    it('sin comparativo la cabecera tiene una sola columna de importes', () => {
        const opts = { ...defaultExportOptions(false), comparative: false, content: { esp: true, er: false, eepn: false, efe: false, notas: false, indicadores: false, analisis: false } }
        const esp = buildSelectedReportSheets(bundle, opts).find(s => s.name === 'ESP')!
